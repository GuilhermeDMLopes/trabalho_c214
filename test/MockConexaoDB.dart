...
class MockConexaoDB {
  static final String nomeTabelaFornecedor = "fornecedorTESTE";
  static final String nomeTabelaProduto = "produtoTESTE";
  static final String nomeTabelaUsuario = "usuarioTESTE";
  static final MockConexaoDB _fornecedorDAO = MockConexaoDB._internal();
  Database? _db;

  factory MockConexaoDB() {
    return _fornecedorDAO;
  }

  MockConexaoDB._internal() {}
  ...
  inicializarDB() async {
    ...
    var db = await openDatabase(localBancoDados,
        version: 1, onCreate: _onCreateFornecedor);
    return db;
  }
  ...
  Future<int> salvarfornecedor(Fornecedor fornecedor) async {
    ...
    int resultado =
        await bancoDados.insert(nomeTabelaFornecedor, fornecedor.toMap());
    return resultado;
  }
  ...
  Future<int> atualizarFornecedor(Fornecedor fornecedor) async {
    var bancoDados = await db;
    return await bancoDados.update(nomeTabelaFornecedor, fornecedor.toMap(),
  ...
  Future<int> removerfornecedor(int id) async {
    var bancoDados = await db;
    return bancoDados
        .delete(nomeTabelaFornecedor, where: "id = ?", whereArgs: [id]);
  }
  ...
  Future<int> atualizarproduto(Produto produto) async {
    var bancoDados = await db;
    return await bancoDados.update(nomeTabelaProduto, produto.toMap(),
  ...
  Future<int> removerproduto(int id) async {
    var bancoDados = await db;
    return bancoDados
        .delete(nomeTabelaProduto, where: "id = ?", whereArgs: [id]);
  }
